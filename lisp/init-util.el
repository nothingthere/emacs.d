...
;;辅助函数
(defun claudio/util-string-empty-p(str)
  "字符串STR是否不含任何字符串.
本来有内置string-empty-p函数，不过需(require 'subr-x)
目前只是用这一个函数，代价挺大."
  (zerop (length str)))

(cl-defun claudio/util-list2string(lst &optional (sep " "))
  "将链表LST转换为字符串，用SEP隔开.
不支持嵌套链表."
  (concatenate
   'string
...
(defun claudio/util-app-running-p(pname)
...
;; (claudio/util-app-running-p "emacs24")

;;;;;;;;;;;;;;;;;buffer操作函数
(cl-defmacro claudio/util-with-save-position+widen(&body body)
...
(defun claudio/util-get-region-or-get-the-line-as-region()
...
(defun claudio/util-current-line-empty-p ()
  "判断当前行是否为空行.
...
(defun claudio/util-at-end-of-line-p()
  "光标所在行有内容，且光标在最后。"
  (and (not (claudio/util-current-line-empty-p))
       (or (eobp)                        ;; 或者是文本末尾
           (char-equal (char-after) ?\n) ;; 后面是换行符（只对linux环境文件有效）
           )))

(defun claudio/util-str-trim-end(str)
  "删除字符串STR末尾的空白字符.
...
;; 链表及原子操作函数
(defmacro claudio/util-sort-symbols(symbols)
  "将所有的symbol按字符顺序排序"
  `(sort ,symbols
         (lambda(x y)
...
(cl-defmacro claudio/util-simple-save-excursion(&body body)
