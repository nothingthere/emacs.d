...
(defvar *claudio/timestamp-insert-timestamp-format*
  (concat *claudio/timestamp-insert-date-format*
          " "
          *claudio/timestamp-insert-time-format*
          )
  "* \\[claudio/timestamp-insert-timestamp]的打印格式 (c.f. format-time-string')."
  )

(defvar *claudio/timestamp-prefix* "@[[" "*时间戳的前缀.")

(defvar *claudio/timestamp-suffix* "]]" "*时间戳后缀.")

(defun claudio/timestamp-instert-time(&optional type)
  ...
  (cl-flet* ((insert-time()
             ...
             (insert-date()
                         (insert (format-time-string
                                  *claudio/timestamp-insert-date-format*
                                  (current-time)))))
    (cl-case type
      ('date (insert-date))
      ('both (insert-date)
...
(defun claudio/timestamp-instert-timestamp()
  ...
  (insert (format-time-string *claudio/timestamp-insert-timestamp-format*
                              (current-time)))
  ...
  (claudio/with-save-position+widen
   (save-match-data
     ...
     (let ((regexp (concat (regexp-quote *claudio/timestamp-prefix*)
                           "\\(.*\\)"
                           (regexp-quote *claudio/timestamp-suffix*))))
       (while (re-search-forward regexp nil t)
         (replace-match (format-time-string
                         *claudio/timestamp-insert-timestamp-format*
                         (current-time))
                        t t nil 1))))))
